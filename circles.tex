...
\end{tikzpicture}
\caption{Loading of image and detection of contours.}
\end{figure}

The circle tracking use

% \begin{figure}
% \centering
% \begin{tikzpicture}[node distance=2cm,scale=0.6, every node/.style={transform shape}]
% 	\node (start) [startstop] {Start};
% 	\node (in1) [io, below of=start] {new path Direction new path Distance new path Pushing};
% 	\node (pro1) [process, below of=in1] {Turn robot to Direction if necessary};
% 	\node (pro2) [process, below of=pro1] {Drive robot Distance steps};
% 	\node (dec1) [decision, below of=pro2, yshift=-0.5cm] {Is Pushing?};
% 	\node (pro3) [process, right of=dec1, xshift=4.1cm] {Back away from can};
% 	\node (dec2) [decision, below of=dec1, yshift=-1.3cm] {Is finished?};
% 	\node (stop) [startstop, below of=dec2, yshift=-0.5cm] {Stop};
% 	\draw [arrow] (start) --                                                                  (in1);
% 	\draw [arrow] (in1)  --                                                                   (pro1);
% 	\draw [arrow] (pro1) --                                                                   (pro2);
% 	\draw [arrow] (pro2) --                                                                   (dec1);
% 	\draw [arrow] (dec1) -- node[anchor=north]      {Yes}                                     (pro3);
% 	\draw [arrow] (dec1) -- node[anchor=east]       {No}                                      (dec2);
% 	\draw [arrow] (pro3) |-                                                                   (dec2);
% 	\draw [arrow] (dec2) -- node[anchor=east]       {Yes}                                     (stop);
% 	\draw [arrow] (dec2) -| node[anchor=north west] {No} ([xshift=-0.25cm]pro1.south west) |- (in1);
% \end{tikzpicture}
% \caption{Robot path execution}
% \label{tikz:robot_behavior}
% \end{figure}
